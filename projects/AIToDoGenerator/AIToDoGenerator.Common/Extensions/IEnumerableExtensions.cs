...
namespace AIToDoGenerator.Common.Extensions
{
    /// <summary>
    ...
    public static class IEnumerableExtensions
    {
        ...
        /// <summary>
        /// Reduces consecutive empty or whitespace strings in the sequence to a single occurrence.
        /// </summary>
        /// <param name="source">The sequence of strings to process.</param>
        /// <returns>
        /// An <see cref="IEnumerable{String}"/> where consecutive empty or whitespace strings are reduced to a single occurrence,
        /// preserving non-empty strings and the first empty string in any consecutive group.
        /// </returns>
        public static IEnumerable<string> ReduceEmptyStrings(this IEnumerable<string> source)
        {
        ...
        /// <summary>
        /// Filters a sequence of strings based on a predicate applied to each string after trimming leading and trailing spaces.
        /// Only strings for which the predicate returns true are included in the result.
        /// </summary>
        /// <param name="source">The sequence of strings to filter.</param>
        /// <param name="filter">
        /// A function to test each trimmed string for a condition. If null, all items are included.
        /// </param>
        /// <returns>
        /// An <see cref="IEnumerable{String}"/> that contains elements from the input sequence that satisfy the condition.
        /// </returns>
        public static IEnumerable<string> WhereByTrim(this IEnumerable<string> source, Func<string, bool> filter)
        {
            var result = new List<string>();

            foreach (var item in source ?? [])
            {
                var cleanItem = new string(item.Where(c => char.IsLetterOrDigit(c) || c == ' ').ToArray());

                if (filter == null || filter(item.RemoveLeftAndRight(' ')))
                {
                    result.Add(item);
                }
            }
