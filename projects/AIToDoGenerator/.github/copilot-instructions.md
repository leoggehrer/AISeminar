# GitHub Copilot Instructions für AIToDoGenerator
...
| **Datenbank erstellen** | `dotnet run --project AIToDoGenerator.ConApp -- AppArg=1,2,x` |

---

## Projektübersicht

AIToDoGenerator ist ein **Template-basiertes Code-Generierungssystem** für .NET/Angular Anwendungen mit Clean Architecture. Der Kern ist ein ausgeklügeltes Code-Generierungssystem, das CRUD-Operationen, API-Controller, Services und UI-Komponenten automatisch aus Entity-Definitionen erstellt.
...
┌───────────────────────────────────────────────────────────────-─┐
│                      Backend Projects                           │
│  AIToDoGenerator.Logic (Entities, EntitySets, DbContext)             │
│  AIToDoGenerator.WebApi (Controllers)                                │
│  AIToDoGenerator.ConApp (CLI, Data Import)                           │
│  AIToDoGenerator.MVVMApp (Desktop UI)                                │
└──────────────────────────────────────────────────────────────-──┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│                     Frontend Project                            │
│  AIToDoGenerator.AngularApp (Components, Services, Models)           │
└─────────────────────────────────────────────────────────────────┘
```

**Kernkonzepte:**
- **Entity-First**: Entities (in `AIToDoGenerator.Logic/Entities/`) sind die einzige Quelle der Wahrheit
...
dotnet run --project AIToDoGenerator.ConApp -- AppArg=1,2,x
```
...
| **AIToDoGenerator.Logic** | Entities, DbContext, EntitySets | ✅ Entities erstellen |
| **AIToDoGenerator.WebApi** | REST API Controller | 🔄 Generiert |
| **AIToDoGenerator.ConApp** | CLI, DB-Migration, CSV-Import | ✅ Import-Logik |
| **AIToDoGenerator.MVVMApp** | Desktop App (Avalonia) | 🔄 Generiert |
| **AIToDoGenerator.AngularApp** | SPA Frontend | 🔄 Generiert |
| **AIToDoGenerator.Common** | Shared Utilities | ✅ Helpers |
...
Erstelle eine partielle Klasse `partial class EntitySet<TEntity>` im Namespace `AIToDoGenerator.Logic.DataContext`. Konfiguriere im Constructor die Zugriffsrechte:

```csharp
//@AiCode
#if GENERATEDCODE_ON && ACCOUNT_ON

namespace AIToDoGenerator.Logic.DataContext;

using AIToDoGenerator.Logic.DataContext.App;
...
Der Generator nutzt **Reflection**, um alle Entities in `AIToDoGenerator.Logic/Entities/` zu finden. Für jede Entity werden dann automatisch erstellt:
...
Überprüfe die `AIToDoGenerator.ConApp`-Projektdatei, um sicherzustellen, dass die gewünschten Datenbank-Defines gesetzt sind:
...
5. **Namespaces:** `AIToDoGenerator.Logic.Entities[.SubFolder]` als Basis
6. **Enums:** Enums werden in eigenen Dateien und im `AIToDoGenerator.Common.Enums[.SubFolder]` abgelegt

### Dateistruktur

| Typ | Pfad | Verwendung |
|-----|------|------------|
| **Stammdaten** | `AIToDoGenerator.Logic/Entities/Data/` | Grundlegende Daten (Kategorien, etc.) |
| **Anwendungsdaten** | `AIToDoGenerator.Logic/Entities/App/` | Geschäftslogik-Daten |
| **Account** | `AIToDoGenerator.Logic/Entities/Account/` | Benutzerverwaltung |
| **Views** | `AIToDoGenerator.Logic/Entities/Views/` | Datenbankviews (ReadOnly) |
...
```csharp
//@AiCode
namespace AIToDoGenerator.Logic.Entities.App
{
...
- Navigation Properties **immer** vollqualifiziert deklarieren, z.B.: 
  `public AIToDoGenerator.Logic.Entities.App.Category? Category { get; set; }`
...
/// </summary>
public AIToDoGenerator.Logic.Entities.App.ParentEntity? ParentEntity { get; set; }
```
...
```csharp
//@AiCode
namespace AIToDoGenerator.Logic.Entities[.SubFolder]
{
    using System.... // Required usings
    using AIToDoGenerator.Logic.Modules.Exceptions;

    partial class EntityName : AIToDoGenerator.Logic.Contracts.IValidatableEntity 
    {
        public void Validate(AIToDoGenerator.Logic.Contracts.IContext context, EntityState entityState)
        {
            bool handled = false;
        ...
        #region partial methods
        partial void BeforeExecuteValidation(ref bool handled, AIToDoGenerator.Logic.Contracts.IContext context, EntityState entityState);
        #endregion partial methods
    }
}
```
...
```csharp
//@AiCode
namespace AIToDoGenerator.Logic.Entities.Views[.SubFolder]
{
    using System.ComponentModel.DataAnnotations;
...
1. **Namespace:** Der Namespace `AIToDoGenerator.ConApp.Apps` ist verpflichtend und darf nicht geändert werden.
...
namespace AIToDoGenerator.ConApp.Apps
{
...
**Dateiname:** `AIToDoGenerator.Logic/DataContext/[SubFolder]/EntityNameSet.Custom.cs`

```csharp
//@AiCode
#if GENERATEDCODE_ON && ACCOUNT_ON

namespace AIToDoGenerator.Logic.DataContext.[SubFolder]
{
...
**Grundregel:** Alle Logik-Erweiterungen (z.B. externe API-Aufrufe, Services, Helper-Klassen) gehören in `AIToDoGenerator.Logic/Modules/`.
...
| Typ | Pfad | Verwendung |
|-----|------|------------|
| **API-Clients** | `AIToDoGenerator.Logic/Modules/ApiClients/` | REST-API Aufrufe zu Fremdsystemen |
| **Services**    | `AIToDoGenerator.Logic/Modules/Services/`   | Geschäftslogik-Services |
| **Helpers**     | `AIToDoGenerator.Logic/Modules/Helpers/`    | Utility-Klassen und Hilfsmethoden |
| **Exceptions**  | `AIToDoGenerator.Logic/Modules/Exceptions/` | Custom Exceptions (bereits vorhanden) |
| **Extensions**  | `AIToDoGenerator.Logic/Modules/Extensions/` | Extension Methods |
| **Validators**  | `AIToDoGenerator.Logic/Modules/Validators/` | Komplexe Validierungslogik |

### Beispiel: REST-API Client

**Dateiname:** `AIToDoGenerator.Logic/Modules/ApiClients/WeatherApiClient.cs`

```csharp
//@AiCode
namespace AIToDoGenerator.Logic.Modules.ApiClients
{
...
**Dateiname:** `AIToDoGenerator.Logic/Modules/Services/OrderProcessingService.cs`

```csharp
//@AiCode
namespace AIToDoGenerator.Logic.Modules.Services
{
    using AIToDoGenerator.Logic.Contracts;
    using AIToDoGenerator.Logic.Entities.App;
    using AIToDoGenerator.Logic.Modules.Exceptions;
...
**Dateiname:** `AIToDoGenerator.Logic/Modules/Helpers/StringHelper.cs`

```csharp
//@AiCode
namespace AIToDoGenerator.Logic.Modules.Helpers
{
...
Die spezifischen Anweisungen für die Erstellung und Strukturierung der Angular-Komponenten befinden sich im Ordner AIToDoGenerator.AngularApp/.github/copilot-instructions.md.
...
1. Datenbank erstellen: `dotnet run --project AIToDoGenerator.ConApp -- AppArg=1,2,x`
...
- **Datenbank testen:** Nutze `AIToDoGenerator.ConApp` für direkte DB-Tests
