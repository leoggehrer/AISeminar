# AIToDoGenerator.McpSrv - Model Context Protocol Server

Ein **Model Context Protocol (MCP) Server** für das AIToDoGenerator-Projekt, der AI-Assistenten direkten Zugriff auf Backend-Funktionen ermöglicht.
...
```bash
# Server starten
dotnet run --project AIToDoGenerator.McpSrv
...
```bash
# AIToDoGenerator.Logic referenzieren
dotnet add reference ../AIToDoGenerator.Logic/AIToDoGenerator.Logic.csproj
```
...
namespace AIToDoGenerator.McpSrv.Servers;
...
```csharp
namespace AIToDoGenerator.McpSrv.Models;
...
- **AIToDoGenerator.Logic** - Backend-Logik und Datenzugriff
...
- [AIToDoGenerator Hauptdokumentation](../README.md)
