...
namespace ProductCatalog.Logic.Modules.CodeGenerator
{
    /// <summary>
        ...
        public static Type[] AllTypes
        {
            get
            {
                var allTypes = AppDomain.CurrentDomain
                                        .GetAssemblies()
                                        .Where(a => a.FullName!.StartsWith($"{nameof(ProductCatalog)}{Common.StaticLiterals.CommonExtension}")
                                                 || a.FullName!.StartsWith($"{nameof(ProductCatalog)}{Common.StaticLiterals.LogicExtension}"))
                                        .SelectMany(assembly =>
                                        {
                                            try
                                            {
