# GitHub Copilot Instructions für ProductCatalog
...
| **Datenbank erstellen** | `dotnet run --project ProductCatalog.ConApp -- AppArg=1,2,x` |

---

## Projektübersicht

ProductCatalog ist ein **Template-basiertes Code-Generierungssystem** für .NET/Angular Anwendungen mit Clean Architecture. Der Kern ist ein ausgeklügeltes Code-Generierungssystem, das CRUD-Operationen, API-Controller, Services und UI-Komponenten automatisch aus Entity-Definitionen erstellt.
...
┌───────────────────────────────────────────────────────────────-─┐
│                      Backend Projects                           │
│  ProductCatalog.Logic (Entities, EntitySets, DbContext)             │
│  ProductCatalog.WebApi (Controllers)                                │
│  ProductCatalog.ConApp (CLI, Data Import)                           │
│  ProductCatalog.MVVMApp (Desktop UI)                                │
└──────────────────────────────────────────────────────────────-──┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│                     Frontend Project                            │
│  ProductCatalog.AngularApp (Components, Services, Models)           │
└─────────────────────────────────────────────────────────────────┘
```

**Kernkonzepte:**
- **Entity-First**: Entities (in `ProductCatalog.Logic/Entities/`) sind die einzige Quelle der Wahrheit
...
dotnet run --project ProductCatalog.ConApp -- AppArg=1,2,x
```
...
| **ProductCatalog.Logic** | Entities, DbContext, EntitySets | ✅ Entities erstellen |
| **ProductCatalog.WebApi** | REST API Controller | 🔄 Generiert |
| **ProductCatalog.ConApp** | CLI, DB-Migration, CSV-Import | ✅ Import-Logik |
| **ProductCatalog.MVVMApp** | Desktop App (Avalonia) | 🔄 Generiert |
| **ProductCatalog.AngularApp** | SPA Frontend | 🔄 Generiert |
| **ProductCatalog.Common** | Shared Utilities | ✅ Helpers |
...
Erstelle eine partielle Klasse `partial class EntitySet<TEntity>` im Namespace `ProductCatalog.Logic.DataContext`. Konfiguriere im Constructor die Zugriffsrechte:
...
using ProductCatalog.Logic.DataContext.App;
...
Der Generator nutzt **Reflection**, um alle Entities in `ProductCatalog.Logic/Entities/` zu finden. Für jede Entity werden dann automatisch erstellt:
...
Überprüfe die `ProductCatalog.ConApp`-Projektdatei, um sicherzustellen, dass die gewünschten Datenbank-Defines gesetzt sind:
...
5. **Namespaces:** `ProductCatalog.Logic.Entities[.SubFolder]` als Basis
6. **Enums:** Enums werden in eigenen Dateien und im `ProductCatalog.Common.Enums[.SubFolder]` abgelegt

### Dateistruktur

| Typ | Pfad | Verwendung |
|-----|------|------------|
| **Stammdaten** | `ProductCatalog.Logic/Entities/Data/` | Grundlegende Daten (Kategorien, etc.) |
| **Anwendungsdaten** | `ProductCatalog.Logic/Entities/App/` | Geschäftslogik-Daten |
| **Account** | `ProductCatalog.Logic/Entities/Account/` | Benutzerverwaltung |
| **Views** | `ProductCatalog.Logic/Entities/Views/` | Datenbankviews (ReadOnly) |
...
- Navigation Properties **immer** vollqualifiziert deklarieren, z.B.: 
  `public ProductCatalog.Logic.Entities.App.Category? Category { get; set; }`
...
/// </summary>
public ProductCatalog.Logic.Entities.App.ParentEntity? ParentEntity { get; set; }
```
...
```csharp
//@AiCode
namespace ProductCatalog.Logic.Entities[.SubFolder]
{
    using System.... // Required usings
    using ProductCatalog.Logic.Modules.Exceptions;

    partial class EntityName : ProductCatalog.Logic.Contracts.IValidatableEntity 
    {
...
```csharp
//@AiCode
namespace ProductCatalog.Logic.Entities.Views[.SubFolder]
{
    using System.ComponentModel.DataAnnotations;
...
1. **Namespace:** Der Namespace `ProductCatalog.ConApp.Apps` ist verpflichtend und darf nicht geändert werden.
...
namespace ProductCatalog.ConApp.Apps
{
...
**Dateiname:** `ProductCatalog.Logic/DataContext/[SubFolder]/EntityNameSet.Custom.cs`

```csharp
//@AiCode
#if GENERATEDCODE_ON && ACCOUNT_ON

namespace ProductCatalog.Logic.DataContext.[SubFolder]
{
...
**Grundregel:** Alle Logik-Erweiterungen (z.B. externe API-Aufrufe, Services, Helper-Klassen) gehören in `ProductCatalog.Logic/Modules/`.
...
| Typ | Pfad | Verwendung |
|-----|------|------------|
| **API-Clients** | `ProductCatalog.Logic/Modules/ApiClients/` | REST-API Aufrufe zu Fremdsystemen |
| **Services**    | `ProductCatalog.Logic/Modules/Services/`   | Geschäftslogik-Services |
| **Helpers**     | `ProductCatalog.Logic/Modules/Helpers/`    | Utility-Klassen und Hilfsmethoden |
| **Exceptions**  | `ProductCatalog.Logic/Modules/Exceptions/` | Custom Exceptions (bereits vorhanden) |
| **Extensions**  | `ProductCatalog.Logic/Modules/Extensions/` | Extension Methods |
| **Validators**  | `ProductCatalog.Logic/Modules/Validators/` | Komplexe Validierungslogik |

### Beispiel: REST-API Client

**Dateiname:** `ProductCatalog.Logic/Modules/ApiClients/WeatherApiClient.cs`

```csharp
//@AiCode
namespace ProductCatalog.Logic.Modules.ApiClients
{
...
**Dateiname:** `ProductCatalog.Logic/Modules/Services/OrderProcessingService.cs`

```csharp
//@AiCode
namespace ProductCatalog.Logic.Modules.Services
{
    using ProductCatalog.Logic.Contracts;
    using ProductCatalog.Logic.Entities.App;
    using ProductCatalog.Logic.Modules.Exceptions;
...
**Dateiname:** `ProductCatalog.Logic/Modules/Helpers/StringHelper.cs`

```csharp
//@AiCode
namespace ProductCatalog.Logic.Modules.Helpers
{
...
Die spezifischen Anweisungen für die Erstellung und Strukturierung der Angular-Komponenten befinden sich im Ordner ProductCatalog.AngularApp/.github/copilot-instructions.md.
...
1. Datenbank erstellen: `dotnet run --project ProductCatalog.ConApp -- AppArg=1,2,x`
...
- **Datenbank testen:** Nutze `ProductCatalog.ConApp` für direkte DB-Tests
