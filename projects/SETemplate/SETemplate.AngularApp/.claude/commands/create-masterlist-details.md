...
Erstelle eine **neue standalone Komponente** für eine **zweispaltige Split-View** von `$ARGUMENTS`.
        ...
        comp.save.subscribe((item: IDetailEntity) => {
          ...
          this.detailService.create(item)
            ...
            .subscribe({
              ...
              error: (err) => console.error('Error creating detail', err)
            });
        });
      },
      ...
      next: (loaded) => {
        ...
        comp.save.subscribe((updated: IDetailEntity) => {
          ...
          this.detailService.update(updated)
            ...
            .subscribe({
              next: () => { comp.close(); this.loadDetails(this.selectedMaster!.id!); },
              error: (err) => console.error('Error updating detail', err)
            });
        });
      },
...
<div class="list-container">
    <div class="container-fluid mt-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3 p-3 page-header">
            ...
            <a routerLink="/dashboard" class="btn btn-link text-secondary p-1"
...
### Suchlogik anpassen (`filteredMasters` Getter)
```typescript
return this.masters.filter(m =>
...
```typescript
filter: 'studentId == @0'   // Beispiel: studentId, departmentId, courseId
```

### Navigation-Properties ergänzen
```typescript
