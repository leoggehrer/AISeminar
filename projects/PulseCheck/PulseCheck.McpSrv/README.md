# PulseCheck.McpSrv - Model Context Protocol Server

Ein **Model Context Protocol (MCP) Server** für das PulseCheck-Projekt, der AI-Assistenten direkten Zugriff auf Backend-Funktionen ermöglicht.
...
```bash
# Server starten
dotnet run --project PulseCheck.McpSrv
...
```bash
# PulseCheck.Logic referenzieren
dotnet add reference ../PulseCheck.Logic/PulseCheck.Logic.csproj
```
...
namespace PulseCheck.McpSrv.Servers;
...
```csharp
namespace PulseCheck.McpSrv.Models;
...
- **PulseCheck.Logic** - Backend-Logik und Datenzugriff
...
- [PulseCheck Hauptdokumentation](../README.md)
