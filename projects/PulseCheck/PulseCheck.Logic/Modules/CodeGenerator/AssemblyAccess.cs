...
namespace PulseCheck.Logic.Modules.CodeGenerator
{
    /// <summary>
        ...
        public static Type[] AllTypes
        {
            get
            {
                var allTypes = AppDomain.CurrentDomain
                                        .GetAssemblies()
                                        .Where(a => a.FullName!.StartsWith($"{nameof(PulseCheck)}{Common.StaticLiterals.CommonExtension}")
                                                 || a.FullName!.StartsWith($"{nameof(PulseCheck)}{Common.StaticLiterals.LogicExtension}"))
                                        .SelectMany(assembly =>
                                        {
                                            try
                                            {
