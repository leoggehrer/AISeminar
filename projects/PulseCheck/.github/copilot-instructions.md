# GitHub Copilot Instructions für PulseCheck
...
| **Datenbank erstellen** | `dotnet run --project PulseCheck.ConApp -- AppArg=1,2,x` |

---

## Projektübersicht

PulseCheck ist ein **Template-basiertes Code-Generierungssystem** für .NET/Angular Anwendungen mit Clean Architecture. Der Kern ist ein ausgeklügeltes Code-Generierungssystem, das CRUD-Operationen, API-Controller, Services und UI-Komponenten automatisch aus Entity-Definitionen erstellt.
...
┌───────────────────────────────────────────────────────────────-─┐
│                      Backend Projects                           │
│  PulseCheck.Logic (Entities, EntitySets, DbContext)             │
│  PulseCheck.WebApi (Controllers)                                │
│  PulseCheck.ConApp (CLI, Data Import)                           │
│  PulseCheck.MVVMApp (Desktop UI)                                │
└──────────────────────────────────────────────────────────────-──┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│                     Frontend Project                            │
│  PulseCheck.AngularApp (Components, Services, Models)           │
└─────────────────────────────────────────────────────────────────┘
```

**Kernkonzepte:**
- **Entity-First**: Entities (in `PulseCheck.Logic/Entities/`) sind die einzige Quelle der Wahrheit
...
dotnet run --project PulseCheck.ConApp -- AppArg=1,2,x
```
...
| **PulseCheck.Logic** | Entities, DbContext, EntitySets | ✅ Entities erstellen |
| **PulseCheck.WebApi** | REST API Controller | 🔄 Generiert |
| **PulseCheck.ConApp** | CLI, DB-Migration, CSV-Import | ✅ Import-Logik |
| **PulseCheck.MVVMApp** | Desktop App (Avalonia) | 🔄 Generiert |
| **PulseCheck.AngularApp** | SPA Frontend | 🔄 Generiert |
| **PulseCheck.Common** | Shared Utilities | ✅ Helpers |
...
Erstelle eine partielle Klasse `partial class EntitySet<TEntity>` im Namespace `PulseCheck.Logic.DataContext`. Konfiguriere im Constructor die Zugriffsrechte:

```csharp
//@AiCode
#if GENERATEDCODE_ON && ACCOUNT_ON

namespace PulseCheck.Logic.DataContext;

using PulseCheck.Logic.DataContext.App;
...
Der Generator nutzt **Reflection**, um alle Entities in `PulseCheck.Logic/Entities/` zu finden. Für jede Entity werden dann automatisch erstellt:
...
Überprüfe die `PulseCheck.ConApp`-Projektdatei, um sicherzustellen, dass die gewünschten Datenbank-Defines gesetzt sind:
...
5. **Namespaces:** `PulseCheck.Logic.Entities[.SubFolder]` als Basis
6. **Enums:** Enums werden in eigenen Dateien und im `PulseCheck.Common.Enums[.SubFolder]` abgelegt

### Dateistruktur

| Typ | Pfad | Verwendung |
|-----|------|------------|
| **Stammdaten** | `PulseCheck.Logic/Entities/Data/` | Grundlegende Daten (Kategorien, etc.) |
| **Anwendungsdaten** | `PulseCheck.Logic/Entities/App/` | Geschäftslogik-Daten |
| **Account** | `PulseCheck.Logic/Entities/Account/` | Benutzerverwaltung |
| **Views** | `PulseCheck.Logic/Entities/Views/` | Datenbankviews (ReadOnly) |
...
```csharp
//@AiCode
namespace PulseCheck.Logic.Entities.App
{
...
- Navigation Properties **immer** vollqualifiziert deklarieren, z.B.: 
  `public PulseCheck.Logic.Entities.App.Category? Category { get; set; }`
...
/// </summary>
public PulseCheck.Logic.Entities.App.ParentEntity? ParentEntity { get; set; }
```
...
```csharp
//@AiCode
namespace PulseCheck.Logic.Entities[.SubFolder]
{
    using System.... // Required usings
    using PulseCheck.Logic.Modules.Exceptions;

    partial class EntityName : PulseCheck.Logic.Contracts.IValidatableEntity 
    {
        public void Validate(PulseCheck.Logic.Contracts.IContext context, EntityState entityState)
        {
            bool handled = false;
        ...
        #region partial methods
        partial void BeforeExecuteValidation(ref bool handled, PulseCheck.Logic.Contracts.IContext context, EntityState entityState);
        #endregion partial methods
    }
}
```
...
```csharp
//@AiCode
namespace PulseCheck.Logic.Entities.Views[.SubFolder]
{
    using System.ComponentModel.DataAnnotations;
...
1. **Namespace:** Der Namespace `PulseCheck.ConApp.Apps` ist verpflichtend und darf nicht geändert werden.
...
namespace PulseCheck.ConApp.Apps
{
...
**Dateiname:** `PulseCheck.Logic/DataContext/[SubFolder]/EntityNameSet.Custom.cs`

```csharp
//@AiCode
#if GENERATEDCODE_ON && ACCOUNT_ON

namespace PulseCheck.Logic.DataContext.[SubFolder]
{
...
**Grundregel:** Alle Logik-Erweiterungen (z.B. externe API-Aufrufe, Services, Helper-Klassen) gehören in `PulseCheck.Logic/Modules/`.
...
| Typ | Pfad | Verwendung |
|-----|------|------------|
| **API-Clients** | `PulseCheck.Logic/Modules/ApiClients/` | REST-API Aufrufe zu Fremdsystemen |
| **Services**    | `PulseCheck.Logic/Modules/Services/`   | Geschäftslogik-Services |
| **Helpers**     | `PulseCheck.Logic/Modules/Helpers/`    | Utility-Klassen und Hilfsmethoden |
| **Exceptions**  | `PulseCheck.Logic/Modules/Exceptions/` | Custom Exceptions (bereits vorhanden) |
| **Extensions**  | `PulseCheck.Logic/Modules/Extensions/` | Extension Methods |
| **Validators**  | `PulseCheck.Logic/Modules/Validators/` | Komplexe Validierungslogik |

### Beispiel: REST-API Client

**Dateiname:** `PulseCheck.Logic/Modules/ApiClients/WeatherApiClient.cs`

```csharp
//@AiCode
namespace PulseCheck.Logic.Modules.ApiClients
{
...
**Dateiname:** `PulseCheck.Logic/Modules/Services/OrderProcessingService.cs`

```csharp
//@AiCode
namespace PulseCheck.Logic.Modules.Services
{
    using PulseCheck.Logic.Contracts;
    using PulseCheck.Logic.Entities.App;
    using PulseCheck.Logic.Modules.Exceptions;
...
**Dateiname:** `PulseCheck.Logic/Modules/Helpers/StringHelper.cs`

```csharp
//@AiCode
namespace PulseCheck.Logic.Modules.Helpers
{
...
Die spezifischen Anweisungen für die Erstellung und Strukturierung der Angular-Komponenten befinden sich im Ordner PulseCheck.AngularApp/.github/copilot-instructions.md.
...
1. Datenbank erstellen: `dotnet run --project PulseCheck.ConApp -- AppArg=1,2,x`
...
- **Datenbank testen:** Nutze `PulseCheck.ConApp` für direkte DB-Tests
